...
savePath = '"J:\Silva''s Lab\P30 Core Center\SSC_Billing Information\RSQ Header Text Files\"';
theDate = datestr(now);
logFileOut = ['J:\Silva''s Lab\P30 Core Center\Core B_Billing Information\MicroCT Machine Logs\LogFromRSQHeader' theDate(1:11) '.txt'];
        ...
        for j = 3:length(dirs2)
            if dirs2(j).isdir == 1
                ...
                for k = length(headerFiles)
                    if strcmp(dirs(i).name,'00004801') == 0
                        ...
                        readouttime = 317; %constant for microct   
